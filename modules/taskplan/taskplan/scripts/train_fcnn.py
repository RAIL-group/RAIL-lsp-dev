...
def get_model_prep_fn_and_training_strs(args):
    print("Training Fcnn for Task Plan... ...")
    model = Fcnn(args)
    prep_fn = taskplan.utilities.utils.preprocess_fcnn_training_data()
...
def train(args, train_path, test_path):
    ...
    for epoch in range(args.num_epochs):
        for index, train_batch in enumerate(train_loader):
            out = model.forward({
                'latent_features': train_batch['node_feats']
            }, device)
            ...
            if index % args.test_log_frequency == 0:
                with torch.no_grad():
                    ...
                    tout = model.forward({
                        'latent_features': test_batch['node_feats']
                    }, device)
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Train Fcnn net with PyTorch.",
