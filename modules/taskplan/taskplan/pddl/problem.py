...
def get_problem(
    ...
    robot_room_coord = get_robots_room_coords(map_data.get_robot_pose(), map_data.rooms, map_data=map_data)
    objects = {
                    ...
                    for from_loc in cnt_names:
                        for to_loc in cnt_names:
                            ...
                            d = (
                                costs["find"]
                                + costs["pick"]
                                + map_data.known_cost[from_loc][to_loc]
                            )
                            if cost_type == "pessimistic":
                                ...
                                if from_loc == "initial_robot_pose":
                                    ...
                                    room_idx = learned_data["partial_map"].org_edge_index[0][
                                        room_idx_pos
                                    ]
                                    from_room_coords = learned_data["partial_map"].node_coords[
                                        room_idx
                                    ]
                                ...
                                if to_loc == "initial_robot_pose":
                                    ...
                                    to_room_coords = learned_data["partial_map"].node_coords[
                                        room_idx
                                    ]
                                ...
                                if (
                                    ...
                                    intermediate_d = pre_compute[
                                        (gen_name_child, from_room_coords, to_room_coords)
                                    ]
                                else:
                                ...
                                if (from_coord, from_room_coords) in grid_cost:
                                    ...
                                    part_from = map_data.known_cost_coords[
    ...
    #     task = '(or (and (exists (?obj1 - item) (and (obj-type-bread ?obj1) (is-toasted ?obj1) (is-at ?obj1 countertop|1|0))) (exists (?obj2 - item) (and (obj-type-plate ?obj2) (is-at ?obj2 countertop|1|0))) (exists (?obj - item) (and (obj-type-mug ?obj) (filled-with-coffee ?obj) (is-at ?obj countertop|1|0)))))'
        ...
        # task = "(rob-at bed|2)"
        # task = "(is-holding cellphone|11)"
        # task = "(is-at waterbottle|5 countertop|1)"
        # task = "(is-at cellphone|11 bed|2)"
        task = "(is-at waterbottle|1|0 countertop|1|0)(is-at cellphone|2|0 bed|2|0)"
    else:
        task = goal_provider(seed, cnt_of_interest, obj_of_interest, objects, goal_type)
    ...
    # print(init_fluents)
    # print("-------------------/n")
    # with open('/data/init_fluents.pkl', "wb") as f:

    #     pickle.dump(init_fluents, f)
    # # dump = json.loads(init_fluents)
    # # print(json.dumps(init_fluents))
    # quit()
    return struct, task
