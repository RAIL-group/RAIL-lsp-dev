...
def get_robot_pose():
    ...
    pose = TransformManager.lookup_transform(
        "body",
        "map",
        when=rospy.Time.now() + rospy.Duration(3),
    )
    return (pose.position.x, pose.position.y, pose.yaw_rad)


# def move_spot_move_base(container_name):
...
def move_spot(current_room, target_room, container_name, executor):
    # executor=SpotSkillsExecutor()
    print(f"{current_room=}, {target_room=}, {container_name=}")
    if current_room == target_room or current_room == "initial_robot_pose":
        return executor.move_spot_move_base(container_name)
    
    success = executor.move_spot_move_base("reverse_"+current_room)
    if not success:
        return False
    
    success = executor.move_spot_move_base(target_room)
    if not success:
        return False
    success = executor.move_spot_move_base(container_name)
    return success
