...
class State:
    ...
    def satisfies_precondition(self, action: Action, relax: bool = False) -> bool:
        ...
        #     print("\nDEBUG Precondition Check for:", action.name)
        #     print("Required positive preconditions:", action._pos_precond)
        #     print("Current fluents:", self.fluents)
        #     print("Required negative preconditions:", action._neg_precond_flipped)
        #     print("Positive check:", action._pos_precond <= self.fluents)
        #     print("Negative check:", self.fluents.isdisjoint(action._neg_precond_flipped))
...
class Operator:
    ...
    def instantiate(self, objects_by_type: Dict[str, List[str]]) -> List[Action]:
        ...
        if self.name == "pickup":
            # For pickup, generate all valid robot-object-location combinations
            # These actions can involve the same location for different objects
            for assignment in itertools.product(*domains):
                binding = {var: obj for (var, _), obj in zip(self.parameters, assignment)}
                grounded_actions.append(self._ground(binding))
            return grounded_actions

        # Standard handling for other operators requiring unique parameters
        for assignment in itertools.product(*domains):
            binding = {var: obj for (var, _), obj in zip(self.parameters, assignment)}
