...
class FCNN(nn.Module):
    ...
    def loss(self, nn_out, data, device='cpu', writer=None, index=None):
        ...
        is_feasible_xentropy = (self.pos * is_feasible_label * -F.logsigmoid(is_feasible_logits) +
    ...
    def get_net_eval_fn(_, network_file, device=None):
        model = FCNN()
        model.load_state_dict(torch.load(network_file, weights_only=True, map_location=device))
        model.eval()
        model.to(device)

        def frontier_net(datum, subgoals):
            datum = FCNN.preprocess_data(datum)
            prob_feasible_dict = {}
