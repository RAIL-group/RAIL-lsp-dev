...
robot_pos = (31, 183)
locations = ["countertop", "sink", "bed", "fridge"]
#Inital Fluents, will be defined elsewhere
fluents = [Fluent("near robot countertop"), ~Fluent("near robot sink"),
           ~Fluent("near robot bed"), ~Fluent("near robot fridge"),
           Fluent("visited robot countertop"), ~Fluent("visited robot sink"),
           ~Fluent("visited robot bed"), ~Fluent("visited robot fridge")]
#Inital visited
visited = {'countertop'}
#Inital state
print('___Inital state___')
state = State(robot_pos, nodes, visited, fluents)
print(state.fluents)
#Robot now drives near the sink
print('___Robot drives near the sink___')
robot_pos = (110, 85)
new_state = State(robot_pos, nodes, visited, fluents)
