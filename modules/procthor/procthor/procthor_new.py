...
class ThorInterface:
    ...
    #     """Create a scene graph from the current scene."""
    #     # Create the graph using the scene data
    #     graph = self.create_scene_graph()

    #     # Compute node embeddings if needed
    #     node_features = graph.get_node_features() if include_node_embeddings else None

    #     # Add additional graph attributes for compatibility
    #     return {
    #         'nodes': {idx: {
    #             'id': node.id,
    #             'name': node.name,
    ...
    #         'node_coords': {idx: node.position for idx, node in graph.nodes.items()},
    #         'node_names': {idx: node.name for idx, node in graph.nodes.items()},
    #         'graph_nodes': node_features,
    #         'graph_edge_index': graph.get_edge_index(),
