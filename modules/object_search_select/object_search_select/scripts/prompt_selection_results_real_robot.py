...
def compute_ucb_bandit_cost(env, planners, random_seed=42):
    ...
    for i in range(NUM_TRIALS):
        ...
        ucb_cost = mean_cost_per_planner - EXPLORATION_C * np.sqrt(np.log(num_trials)
        ...
        cost = np.loadtxt(cost_file)[min_idx] / RESOLUTION
...
def compute_lbcost_wavg(env, chosen_planner, env_seed, prob_shortcut=0):
    ...
    true_costs = np.loadtxt(cost_file) / RESOLUTION
    lb_costs = np.loadtxt(lb_costs_file) / RESOLUTION
...
def compute_lb_selection_cost(env, planners, prob_shortcut=0, random_seed=42):
    ...
    for i in range(NUM_TRIALS):
        ...
        bandit_cost = (mean_cost_per_planner[0] - EXPLORATION_C * bandit_exploration_magnitude)
...
def get_planner_costs_by_seed(env, planners):
    """Gets the cost of each planner for each map seed in an environment."""
    seeds = np.arange(*env_seeds[env])
    all_costs = []
    for seed in seeds:
        costs_per_planner = []
        for j, chosen_planner in enumerate(planners):
            cost_file = Path(args.save_dir) / f'cost_{chosen_planner}_all_{all_planners}_{env}_{seed}.txt'
            cost = np.loadtxt(cost_file) / RESOLUTION
            costs_per_planner.append(cost[j])
        all_costs.append(costs_per_planner)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_seeds', type=int, nargs='+', default=[0])
    parser.add_argument('--num_seeds', type=int, default=NUM_TRIALS)
    ...
    planner_names = [
        ...
        'LLM-DIRECT/P-DIRECT/GPT-5',
        # 'LLM-DIRECT/P-DIRECT/Gemini'
    ]
    planner_plot_order = [0, 1, 2]
    our_planner_gpt_idx = [0, 1]
    # our_planner_gemini_idx = [4, 5, 6]
    direct_planner_gpt_idx = 2
    # direct_planner_gemini_idx = 8
    # optim_planner_idx = 0

    envs = ['home']
    env_names = ['Home']
    env_seeds = {'home': (args.start_seeds[0], args.start_seeds[0] + args.num_seeds)}
    ...
    for i, env in enumerate(envs):
        ...
        # print(f'Our Best Planner GPT: {p_names[our_planner_gpt_idx][our_best_planner_gpt]}, '
        #       f'Cost: {our_best_planner_gpt_cost:.2f}')
        ...
        # print(f'Our Worst Planner GPT: {p_names[our_planner_gpt_idx][our_worst_planner_gpt]}, '
        #       f'Cost: {our_worst_planner_gpt_cost:.2f}')
        # print('----------------------------------------------------')
        # directllm_gpt_cost = env_planner_costs[i][direct_planner_gpt_idx]
        # optimistic_cost = env_planner_costs[i][optim_planner_idx]
        # percent_improve_gpt_our_best_vs_directllm = (
        #     directllm_gpt_cost - our_best_planner_gpt_cost) / directllm_gpt_cost * 100
        # print('Improvement GPT: Our Best Planner wrt Full-LLM Planner: '
        #       f'{percent_improve_gpt_our_best_vs_directllm:.1f}%')
        # percent_improve_gpt_our_worst_vs_directllm = (
        #     directllm_gpt_cost - our_worst_planner_gpt_cost) / directllm_gpt_cost * 100
        # print('Improvement GPT: Our Worst Planner wrt Full-LLM Planner: '
        #       f'{percent_improve_gpt_our_worst_vs_directllm:.1f}%')
        # percent_improve_gpt_our_best_vs_optimistic = (
        #     optimistic_cost - our_best_planner_gpt_cost) / optimistic_cost * 100
        # print('Improvement GPT: Our Best Planner wrt Optimistic Planner: '
        #       f'{percent_improve_gpt_our_best_vs_optimistic:.1f}%')
        # percent_improve_gpt_our_worst_vs_optimistic = (
        #     optimistic_cost - our_worst_planner_gpt_cost) / optimistic_cost * 100
        # print('Improvement GPT: Our Worst Planner wrt Optimistic Planner: '
        #       f'{percent_improve_gpt_our_worst_vs_optimistic:.1f}%')
        # print('----------------------------------------------------')
        ...
        # percent_improve_gemini_our_best_vs_optimistic = (
        #     optimistic_cost - our_best_planner_gemini_cost) / optimistic_cost * 100
        # print('Improvement Gemini: Our Best Planner wrt Optimistic Planner: '
        #       f'{percent_improve_gemini_our_best_vs_optimistic:.1f}%')
        # percent_improve_gemini_our_worst_vs_optimistic = (
        #     optimistic_cost - our_worst_planner_gemini_cost) / optimistic_cost * 100
    ...
    # Save the order of planners by performance
    for env in envs:
