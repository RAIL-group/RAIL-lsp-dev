...
def test_robot_position():
    ...
    robot = graphs.RobotData(robot_id = 1, position=[0.0, 0.0], cur_vertex=start_node)
...
#     start, goal, graph, robots = graphs.linear_graph_unc() 
#     block_probs = [0.0, 0.5, 0.9, 1.0]
#     for prob in block_probs:
#         edge12 = [edge for edge in graph.edges if edge.id == (1, 2)][0]
...
#    start_node, goal_node, graph, robots = graphs.s_graph_unc() # edge 34 is blocked.
