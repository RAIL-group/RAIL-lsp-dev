...
def test_groundstate_transition_lgraph():
    ...
    state = gstate_dec.GroundState(graph=l_graph, goalID=goal.id, robot=robot)
    assert len(state.get_actions()) == len(state.actions) == 1
    ...
    assert len(state1.actions) == len(state1.get_actions()) == 1
    
    state2 = list(state_prob_cost.keys())[1] # blocked state
    assert state2.robot.need_action == True
    assert state2.robot.remaining_time == 0.0
    assert len(state2.actions) == len(state2.get_actions()) == 1
...
def test_groundstate_transition_dgraph():
    start, goal, graph = graphs.disjoint_unc()
    robot = Robot(position=[start.coord[0], start.coord[1]], cur_node=start.id, at_node=True)
    state = gstate_dec.GroundState(graph=graph, goalID=goal.id, robot=robot)
    assert len(state.get_actions()) == len(state.actions) == 2
    assert state.robot.need_action == True
    assert state.history.get_data_length() == len(state.graph.vertices)
    ...
    ## second transition
    assert state1.get_actions()[0].target == 2
    state_prob_cost = state1.transition(state1.get_actions()[0])
    assert len(state_prob_cost) == 1
    state2 = list(state_prob_cost.keys())[0]
    assert state2.robot.need_action == True
    assert state2.robot.last_node == 2
    assert len(state2.actions) == len(state2.get_actions()) == 1
    ...
    state_prob_cost = state2.transition(state2.get_actions()[0])
    assert len(state_prob_cost) == 2
    state3_T = list(state_prob_cost.keys())[0]
    assert state3_T.history.get_action_outcome(state2.get_actions()[0]) == param.EventOutcome.TRAV
    assert state3_T.robot.last_node == 7
    assert len(state3_T.get_actions()) == 1
    assert state3_T.get_actions()[0].target == 3
    
    state3_B = list(state_prob_cost.keys())[1]
    assert state3_B.history.get_action_outcome(state2.get_actions()[0]) == param.EventOutcome.BLOCK
    assert len(state3_B.get_actions()) == 1
    assert state3_B.robot.last_node == 7
    assert state3_B.robot.need_action == True
    assert state3_B.get_actions()[0].target == 2
    
    ## fourth transition - from state3_T
    state_prob_cost = state3_T.transition(state3_T.get_actions()[0])
    assert len(state_prob_cost) == 1
