...
def test_stateDecPrior_transition_lgraph():
    print()
    ...
    state = dec_prior.StateDecPrior(graph=l_graph, goalIDs=[g.id for g in goals], ugvs=ugvs, drones=drones)
    ...
    # the fourth transition - assign action 4 to the ugv then move 
    ...
    # the 8th transition - handle ugv's move
...
def test_stateDecPrior_transition_1uav1ugv_dg():
    print()
    starts, goals, graph = graphs.disjoint_unc()
    uavs = [Robot(position=[0.0, 0.0], cur_node=starts[0].id, robot_type=param.RobotType.Drone) for _ in range(1)]
    ugvs = [Robot(position=[0.0, 0.0], cur_node=starts[0].id) for _ in range(1)]
    ...
    assert state1.get_actions()[0].target == 8
    ...
    # assert state2_p.heuristic == pytest.approx(2.0/param.VEL_RATIO, 0.05)
    ...
    # the 4th transition - assign action to the ugv and move
    assert state3_p.get_actions()[0].rtype == param.RobotType.Ground
    assert state3_p.get_actions()[0].target == 8
    state_prob_cost = state3_p.transition(state3_p.get_actions()[0])
    assert len(state_prob_cost) == 2
    state4_p = list(state_prob_cost.keys())[0]
    ...
    # the 6th transition - assign action to the ugv then move, the drone reach goal first
    ...
    assert state7_p.get_actions()[0].target == 8
    state_prob_cost = state7_p.transition(state7_p.get_actions()[0])
    assert len(state_prob_cost) == 2
    state8_p = list(state_prob_cost.keys())[0]
    assert state8_p.get_actions()[0].rtype == param.RobotType.Ground
    ...
    # assert state8_p.action_cost == 2.0/param.VEL_RATIO

    # the 9th transition - assign action to the ugv 
    ...
    # the 10th transition - assign action to the uav, then move
    ...
    # the 11th transition - assign action to the uav, (staying at goal) then move
...
def test_stateDecPrior_transition_2uav1ugv_dg():
    print()
    num_uav = 2
    starts, goals, graph = graphs.disjoint_unc()
    uavs = [Robot(position=[0.0, 0.0], cur_node=starts[0].id, robot_type=param.RobotType.Drone) for _ in range(num_uav)]
    ugvs = [Robot(position=[0.0, 0.0], cur_node=starts[0].id) for _ in range(1)]
    ...
    # the first transition - assign action to the first drone
    assert state.get_actions()[0].rtype == param.RobotType.Drone
    assert state.get_actions()[0].target == 5
    state_prob_cost = state.transition(state.get_actions()[0])
    assert len(state_prob_cost) == 1
    state1 = list(state_prob_cost.keys())[0]
    assert state1.get_actions()[0].rtype == param.RobotType.Drone
    assert state1.action_cost == 0.0
    assert state1.heuristic == 8.0
    assert state1.ugvs[0].need_action == True
    assert state1.uavs[0].need_action == False
    assert state1.uavs[1].need_action == True
    assert len(state1.get_actions()) == len(state1.state_actions) == 3
    
    
    # the second transition - assign action to the second drone
    assert state1.get_actions()[0].rtype == param.RobotType.Drone
    ...
    assert state2_p.action_cost == 0.0
    assert state2_p.heuristic == 8.0
    assert state2_p.ugvs[0].need_action == True
    assert state2_p.uavs[0].need_action == False
    assert state2_p.uavs[1].need_action == False    
    assert len(state2_p.get_actions()) == len(state2_p.state_actions) == 1

    # the third transition - assign action to the ugv
    assert state2_p.get_actions()[0].rtype == param.RobotType.Ground
    ...
    assert state3_p.action_cost == 2.0/param.VEL_RATIO
    assert state3_p.ugvs[0].need_action == True
    assert state3_p.uavs[0].need_action == True
    assert state3_p.uavs[1].need_action == False    
    assert len(state3_p.get_actions()) == len(state3_p.state_actions) == 2
    ...
    state4_p = list(state_prob_cost.keys())[0]
    assert state4_p.get_actions()[0].rtype == param.RobotType.Ground
    assert state4_p.action_cost == 0.0
    assert state4_p.ugvs[0].need_action == True
    assert state4_p.uavs[0].need_action == False
    assert state4_p.uavs[1].need_action == False    
    assert len(state4_p.get_actions()) == len(state4_p.state_actions) == 1

    # the 5th transition - assign action to the ugv, then move
    assert state4_p.get_actions()[0].rtype == param.RobotType.Ground
    ...
    state5_p = list(state_prob_cost.keys())[0]
    assert state5_p.get_actions()[0].rtype == param.RobotType.Drone
    assert state5_p.action_cost == 4.0/param.VEL_RATIO
    assert state5_p.ugvs[0].need_action == True
    assert state5_p.uavs[0].need_action == True
    assert state5_p.uavs[1].need_action == False    
