...
class StateDecPrior(object):
    def __init__(self, graph=None, goalIDs=[], ugvs=[], drones=[], 
                 iscopy=False, n_maps=100, use2AG=False, max_uanum=3, spolicy_rollouts=500):
    ...
    def copy(self):
        ...
        new_state.spolicy_rollouts = self.spolicy_rollouts
        # new_state.sampling_maps = self.sampling_maps
        if self.use_2AG:
        ...
        if self.uavs != []:
            ...
            new_state.uavs = []
            new_state.uav_actions = []
        assert new_state.sampling_maps == 80
        return new_state
        
    def transition(self, action):
        # print(f"Transition with action: {action}")
...
def update_policy(state, ugv_idx):
    C = 200.0
    max_depth = 10
    useOptHeur = True
    revisit_pen = 20.0
    ...
    sctpstate = core.SCTPState(graph=new_graph, goalID=state.goalIDs[ugv_idx], robot=robot, \
                            n_maps=state.sampling_maps, revisit_pen=revisit_pen)
    if sctpstate.is_goal_state:
        state.noway2goal = True
        return None
    assert state.spolicy_rollouts == 300
    action, _, [ordering, costs], _, _ = pouct_planner.core.po_mcts(sctpstate, \
                    n_iterations=state.spolicy_rollouts, C=C, depth= max_depth, \
                    rollout_fn=core.sctp_rollout3)
    action.update_robotID(ugv_idx)
    action.update_pose((state.ugvs[ugv_idx].cur_pose[0], state.ugvs[ugv_idx].cur_pose[1]))
    state.s_policy_time += (time.perf_counter() - start_time)
    return action
...
def get_ugv_belief(state, last_nodes, robot_idx, last_edges): # need to work on this.
    ...
    if vertex_status == param.EventOutcome.BLOCK:
        ...
        if state.ugvs[robot_idx].last_node == state.goalIDs[robot_idx]:
            ...
            if len(neighbors) > 1:
                act = get_ugv_action(state, robot_idx)
                if act:
                    state.ugvs_actions[robot_idx] = [act]
                else:
                    state.noway2goal = True
...
def get_new_ugv_node(state, robot_idx, last_node=None, blocked=False):
    ...
    for i, robot in enumerate(new_state.ugvs):
        if i != robot_idx and not robot.at_node: # reset if it is in middle of action
            ...
            act = get_ugv_action(new_state, i)
            if act:
                act.update_pose((robot.cur_pose[0], robot.cur_pose[1]))
                act.update_robotID(i)
                new_state.ugvs_actions[i] = [act]
            else:
