...
class JSCTPState(object):
    def __init__(self, graph=None, goalIDs=[], robots=[], drones=[], iscopy=False, n_maps=100):
        ...
        self.robots_actions = []
        assert param.ADD_IV == False
              
        if not iscopy:
            assert graph is not None
            assert robots != []
            assert len(goalIDs) == len(robots)
            self.graph = graph
            ...
            for robot in self.robots:
                ...
                if robot.at_node:
                    ...
                    self.visited_vertices[robot.last_node] = 1
    ...
    def copy(self):
        new_state = JSCTPState(iscopy=True)
    ...
    def update_heuristic(self):
        ...
        for robot in self.robots:
            if robot.at_node == True:
                ...
                if robot.last_node in block_pois:
                    neighbors = [node for node in self.graph.pois if node.id == robot.last_node][0].neighbors
                    other_side = [n for n in neighbors if n != robot.pl_vertex][0]
                    edges.append([robot.last_node, other_side])
            else:
                all_edges.add(robot.last_node)
                all_edges.add(robot.action.target)
                if robot.last_node in block_pois:
                    neighbors = [node for node in self.graph.pois if node.id == robot.last_node][0].neighbors
                    other_side = [n for n in neighbors if n != robot.action.target][0]
                    edges.append([robot.last_node, other_side])
                if  robot.action.target in block_pois:
                    neighbors = [node for node in self.graph.pois if node.id == robot.action.target][0].neighbors
        ...
        self.heuristic = param.NOWAY_PEN if self.heuristic < 0.0 else self.heuristic 
